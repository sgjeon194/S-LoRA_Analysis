...
for _ in range(5):
    ...
    dispatch_bgmv(shrink_result, X, A_batched, a_start, a_len, a_loc, batch_req_bin, 0, a_scaling, \
        lora_stream.cuda_stream if use_multistream else torch.cuda.default_stream().cuda_stream) # (inputsize, 4096) * (4096, max_rank)
    torch.cuda.nvtx.range_pop()

    torch.cuda.nvtx.range_push("Expand")
    dispatch_bgmv(expand_result, shrink_result, B_batched, a_start, a_len, a_loc, batch_req_bin, 0, a_scaling, \
        lora_stream.cuda_stream if use_multistream else torch.cuda.default_stream().cuda_stream) # (inputsize, 4096) * (4096, 4096)
    torch.cuda.nvtx.range_pop()
