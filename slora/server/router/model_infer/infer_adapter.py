...
class InferAdapter:
    ...
    def load_lora_A(self, adapter, loc, prefetch=False):
        ...
        for i in range(adapter.network_config["num_hidden_layers"]):
            ...
            #print(f"\t\tLora A is moved to self.mem_manager.keybuffer. Is this on cuda? : {self.mem_manager.key_buffer[i][loc].is_cuda}")
        ...
        if prefetch:
            ...
            for adapter in adapters:
                if adapter is not None and adapter.lora_dir not in self.idx_map:
                    ...
                    tot_size += adapter.r * 4
            # mark_end("load scan")
            print(f"\n\tload {len(new_adapters)} adapters, {len(self.adapter_dirs) + len(new_adapters)} in total\n")
